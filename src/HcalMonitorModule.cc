...
void HcalMonitorModule::analyze(const edm::Event& e, const edm::EventSetup& eventSetup){
  ...
  // Do we need to revise digiOK_ to allow SubdetStatus checks when not all subdetectors are bad?
  if (digiOK_ && rawOK_)
    {
  ...
  if ((checkHB_ && HBpresent_==0) &&
      ...
      (checkHF_ && HFpresent_==0))
    {
      LogWarning("HcalMonitorModule")<<" No valid Hcal subdetector inforamtion found"<<endl;
